...
// Variables Globales
var videos = [];
var ytplayer; // FUNCIONES PARA MANEJAR LOS ARCHIVOS DE YOUTUBE 
...
var play_video = function play_video(evento) {
  var video = this;
  var titulo = "",
      videoId = "";
  ...
  if (!video.className.includes("selected")) {
    ...
    titulo = parseInt(titulo.slice(-1)); // let  videoyt = document.getElementById("frameyt");
    ...
    if ([2, 0].includes(ytplayer.getPlayerState())) ytplayer.playVideo();else ytplayer.pauseVideo();
  }
};
...
function onPlayerReady() {
  console.log("entre a  onPlayerReady"); //Ajustar volumen
...
function create_cards() {
  ...
  for (var i = 0; i < videos.length; i++) {
    ...
    titulo.id = "video-" + i; //-------------------prueba para extraer el titulo del video----
  ...
  for (var _i = 0; _i < cells.length; _i++) {
    cells[_i].addEventListener("click", play_video);
  }
